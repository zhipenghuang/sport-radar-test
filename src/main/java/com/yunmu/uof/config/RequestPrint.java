...
public class RequestPrint {
     ...
     * 方法调用前触发 记录开始时间
     */
    @Before("execution(* com.yunmu.uof.controller..*.*(..))")
     ...
     * 方法调用后触发 记录结束时间
     */
    @After("execution(* com.yunmu.uof.controller..*.*(..))")
     ...
     * 环绕触发
     */
    @Around("execution(* com.yunmu.uof.controller..*.*(..))")
    public Object doAround(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (request instanceof RepeatedlyReadRequestWrapper && request.getMethod().equalsIgnoreCase("post") && StringUtils.isNotBlank(request.getContentType()) && request.getContentType().contains("application/json")) {
    ...
    private void printOptLog() {
        ...
        log.info("请求地址：" + requestPath + ",请求头：" + requestHeader + ",请求时间：" +
                dateTime.toString("yyyy-MM-dd HH:mm:ss.sss") + ",花费时间：" + (endTimeMillis - startTimeMillis)
                + "ms" + ",请求参数：" + inputParamString + ",请求结果：" + gson.toJson(outputParamMap));
    }
}
